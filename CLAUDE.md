...
- Phase 12 — mobile `HomeScreen.js` (the whole Home tab: statistics, recent founds/losts, categories, social, empty state) re-skinned **skeuomorphic**: done. This is a deliberate, screen-scoped departure from the flat/borderless treatment Phases 8-11 established — every other mobile screen keeps that house style, and the two are meant to coexist, not to be reconciled. Surfaces here are lit physical objects: beveled slabs casting real shadows on a lit page sheet, glossy domed tiles/pills/buttons, a groove cut between the Found and Lost halves of one slab, poster cards mounted in frames behind glass, categories as button caps in a recessed tray, and type either embossed off its surface or stamped into it.
  - Recipes live in [skeuomorph.js](mobile/src/theme/skeuomorph.js) and are **lighting only** — neutral white/black alpha rims, gloss gradients, shadows — layered over the same `colorTokens` palette, so the design-token rule still holds (`skeuoShadow`, `bevelRim`, `glassRim`, `insetWell`, `groove`, `embossText`/`engraveText`, `glossColors`/`domeGlossColors`/`glassSheenColors`, `pageWashColors`). Every recipe has a light and a dark variant: highlights weak + shades strong in dark, the reverse in light.
  - The reflection layer is [SkeuoGloss.js](mobile/src/components/SkeuoGloss.js) (`plate`/`dome`/`glass` variants), rendered as the *first* child of a surface. It clips itself instead of asking the parent to clip, because a parent with `overflow: 'hidden'` loses its iOS drop shadow — same reason `foundLostStrip` dropped its `overflow` and the poster card split into a `posterCard` frame + `posterClip` print.
  - Two Phase 9/10 constraints still apply and shaped this: icon tiles with translucent tints (`statSegmentIcon`, `bigStatCardIcon`) get bevel + gloss but **no** `elevation`, since translucent background + elevation renders as an octagon on Android (`socialIconCircleShadow`'s opaque-wrapper workaround is the other half of that); and `statsPanelGlass` now cancels the whole slab treatment (fill, padding, bevel, shadow), not just the fill, so no rim is drawn around the transparent stats wrapper floating over the world map.
  - `getElevation` is gone from this screen — `skeuoShadow(isDark, 1|2|3)` replaces it everywhere it was used (poster pills, social circles).
