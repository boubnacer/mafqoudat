...
- `surfaceCard`: `#EDEFF6` / `#171B22` — variant of `surfaceRaised` for cards sitting directly on flat `surfaceBase` w/ no panel/gradient between them (plain `surfaceRaised` was only ~3% off `surfaceBase` in light mode, card nearly invisible). Dark mode unchanged (same value as `surfaceRaised` — already enough separation there). Used by the posts list card only (`Post.js`/`PostCardSkeleton.jsx` grid+list layouts, mobile `PostsListScreen.js`/`MyPostsScreen.js` `postCard`/`postCardSkeleton`) — other `surfaceRaised`-based cards (dashboard panels, hero) sit on a blurred-gradient or non-flat backdrop already and don't have the problem, so leave those on `surfaceRaised`.
