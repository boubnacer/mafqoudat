  ...
  endpoints: (builder) => ({
    ...
    getDashboard: builder.query({
      ...
      transformResponse: (responseData, meta, arg) => {
        ...
        // Transform recentFounds to ensure proper field mapping
        const transformedRecentFounds = recentFounds?.map(post => {
          const transformed = {
            ...post,
            // Ensure categoryname is properly mapped
            categoryname: post.categoryname || post.category?.code || 'ELECTRONICS',
            // Ensure createdAt is properly mapped
            createdAt: post.createdAt || post.created_at,
            // Ensure other fields are properly mapped
            cityName: post.cityName || post.city?.labels?.[language] || post.city?.labels?.en,
            cityLabels: post.cityLabels || post.city?.labels,
            countryname: post.countryname || post.country?.code || 'MOROCCO',
            countryLabels: post.countryLabels || post.country?.labels,
          };
          
          // Debug logging for transformation
          console.log('Dashboard transform - recentFounds post:', {
            original: { categoryname: post.categoryname, createdAt: post.createdAt, category: post.category?.code },
            transformed: { categoryname: transformed.categoryname, createdAt: transformed.createdAt }
          });
          
          return transformed;
        }) || [];

        // Transform recentLosts to ensure proper field mapping
        const transformedRecentLosts = recentLosts?.map(post => {
          const transformed = {
            ...post,
            // Ensure categoryname is properly mapped
            categoryname: post.categoryname || post.category?.code || 'ELECTRONICS',
          ...
          // Debug logging for transformation
          console.log('Dashboard transform - recentLosts post:', {
            original: { categoryname: post.categoryname, createdAt: post.createdAt, category: post.category?.code },
            transformed: { categoryname: transformed.categoryname, createdAt: transformed.createdAt }
          });
        ...
        // Transform trendingPost to ensure proper field mapping
        const transformedTrendingPost = trendingPost ? {
          ...trendingPost,
          categoryname: trendingPost.categoryname || trendingPost.category?.code || 'ELECTRONICS',
          createdAt: trendingPost.createdAt || trendingPost.created_at,
          cityName: trendingPost.cityName || trendingPost.city?.labels?.[language] || trendingPost.city?.labels?.en,
          cityLabels: trendingPost.cityLabels || trendingPost.city?.labels,
          countryname: trendingPost.countryname || trendingPost.country?.code || 'MOROCCO',
          countryLabels: trendingPost.countryLabels || trendingPost.country?.labels,
        } : null;

        // Debug logging for trending post transformation
        if (trendingPost) {
          console.log('Dashboard transform - trendingPost:', {
            original: { categoryname: trendingPost.categoryname, createdAt: trendingPost.createdAt, category: trendingPost.category?.code },
            transformed: { categoryname: transformedTrendingPost.categoryname, createdAt: transformedTrendingPost.createdAt }
          });
        }

        return {
          trendingPost: transformedTrendingPost,
          recentFounds: transformedRecentFounds,
          recentLosts: transformedRecentLosts,
          totalFounds,
          totalLosts,
          totalPosts,
          formattedLocations,
