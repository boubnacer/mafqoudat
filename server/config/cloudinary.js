...
// Upload image to Cloudinary with caching
const uploadToCloudinary = async (file, folder = 'mafqoudat', options = {}) => {
  try {
    ...
    console.log('📤 Cloudinary upload completed and cached');
    return uploadResult;
  } catch (error) {
...
const deleteFromCloudinary = async (public_id) => {
  try {
    if (public_id) {
      ...
      await cacheService.invalidatePattern(`cloudinary:upload:*:${public_id.split('/').pop()}`);
